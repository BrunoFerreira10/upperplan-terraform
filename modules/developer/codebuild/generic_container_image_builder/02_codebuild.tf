...
resource "aws_codebuild_project" "ecr" {
  name         = "${var.env}_${var.shortname}_${var.project_name}"
  service_role = aws_iam_role.codebuild.arn

  # artifacts {
  #   type      = "S3"
  #   location  = var.project_bucket.bucket
  ...
  artifacts {
    ...
    location  = var.artifact_file_name != "" ? var.project_bucket.bucket : null
  ...
  source_version = "${var.trigger_branch}"

  logs_config {
    cloudwatch_logs {
      group_name = "/aws/codebuild/${var.env}_${var.shortname}_${var.project_name}"
    }
  }
}
...
resource "aws_codebuild_webhook" "this" {
  project_name = "${var.env}_${var.shortname}_${var.project_name}"
  build_type   = "BUILD"
  filter_group {
    filter {
      type    = "EVENT"
