## --------------------------------------------------------------------------------------------------------------------
## Lambda Function - Criação da função lambda via arquivo python.
## --------------------------------------------------------------------------------------------------------------------
...
resource "aws_lambda_function" "codedeploy_trigger_lambda" {
  function_name = "${var.env}_CodeDeployTriggerLambda_${var.shortname}"
  ...
  environment {
    variables = {
      APPLICATION_NAME      = "${var.env}_${var.shortname}_app"
      DEPLOYMENT_GROUP_NAME = "${var.env}_${var.shortname}_app"
    }
  }
}
